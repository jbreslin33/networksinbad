...
private:
	...
	void setupBody(SceneManager* sceneMgr)
	{
		...
		LogString("server setupBody x: %f", mBodyNode->getPosition().x);
		LogString("server setupBody y: %f", mBodyNode->getPosition().y);
		LogString("server setupBody z: %f", mBodyNode->getPosition().z);
	...
	void updateBody(Real deltaTime)
	{
		...
		if (!mKeyDirection.isZeroLength())
		{
			...
			//LogString("server updateBody");

            //LogString("server xAxis.x %f", mCameraNode->getOrientation().xAxis().x);
			...
			LogString("updateBody x %f", mBodyNode->getPosition().x);
			LogString("updateBody y %f", mBodyNode->getPosition().y);
			LogString("updateBody z %f", mBodyNode->getPosition().z);
		}
	}
